...
from om4_dymos.options import StateOptions, TimeOptions, ControlOptions
...
phase = Phase(
    ...
    time_options=TimeOptions(units='s', targets=['t']),
    state_options={
...
phase.transcription = RadauNew(num_segments=3, order=3, compressed=False)
...
if phase.connections:
    for conn in phase.connections:
        ...
        # Print connections involving controls
        if 'control' in src or 'control' in tgt:
            print(f'  {src} -> {tgt}')
