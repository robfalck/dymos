...
class RadauTranscription(TranscriptionBase):
    """
    Radau Pseudospectral Method Transcription for OM4.
    """
    ...
    def build_spec(self, phase_options: Any) -> tuple[Dict[str, Any], list[dict]]:
        ...
        # We need node_ptau (tau spacing over the whole phase) for TimeComp.
        # For an uncompressed, evenly-spaced segment grid:
        ...
        for i in range(self.num_segments):
            ...
            v1 = segment_ends[i + 1]
            seg_len = v1 - v0
            # map x_col_seg [-1, 1] into [v0, v1]
            mapped_col = v0 + 0.5 * seg_len * (x_col_seg + 1.0)
            node_ptau_col.extend(mapped_col)
            # The derivative of phase tau wrt segment tau is half the segment length in phase space
        ...
        # 2. INDEPENDENT VARIABLES COMPONENT
        ivc_outputs = {}
        ...
        for name, opts in phase_options.state_options.items():
            ivc_outputs[f'states:{name}'] = ContinuousVar(
                shape=(num_disc_nodes, *opts.shape),
                val=opts.initial_val,
                units=opts.units,
            )
        ...
        # Route time to ODE targets
        for tgt in t_opts.targets:
            conns.append({'src': 'time.t', 'tgt': f'ode.{tgt}'})
        ...
        # 5. ODE COMPONENT (Evaluated at the collocation nodes)
        subs['ode'] = phase_options.ode_class(num_nodes=num_col_nodes)

        for name, opts in phase_options.state_options.items():
            for target in opts.targets:
                conns.append(
                    {'src': f'state_interp.state_col:{name}', 'tgt': f'ode.{target}'}
                )

        # 6. COLLOCATION COMP
        if phase_options.state_options:
            subs['collocation'] = CollocationComp(
                num_col_nodes=num_col_nodes, state_options=phase_options.state_options
            )
            conns.append({'src': 'time.dt_dstau', 'tgt': 'collocation.dt_dstau'})
            for name, opts in phase_options.state_options.items():
                conns.append(
                    {
                        'src': f'state_interp.staterate_col:{name}',
                        'tgt': f'collocation.f_approx:{name}',
                    }
                )
                conns.append(
                    {
                        'src': f'ode.{opts.rate_source}',
                        'tgt': f'collocation.f_computed:{name}',
                    }
                )
