...
def ballistic_rocket_max_range(transcription='gauss-lobatto', num_segments=8, transcription_order=3,
    ...
    p.model.add_subsystem('boost', phase)

    phase.set_time_options(fix_initial=True, duration_bounds=(.5, 10))

    phase.set_state_options('x', fix_initial=True, fix_final=False)
    phase.set_state_options('y', fix_initial=True, fix_final=True)
    phase.set_state_options('vx', fix_initial=True, fix_final=False)
    phase.set_state_options('vy', fix_initial=True, fix_final=False)
    phase.set_state_options('mprop', fix_initial=True, fix_final=False)

    phase.add_design_parameter('thrust', units='N', opt=False, val=2000.0)
    phase.add_design_parameter('theta', units='deg', opt=False, val=90.0)
    phase.add_design_parameter('mstruct', units='kg', opt=False, val=100.0)
    phase.add_design_parameter('g', units='m/s**2', opt=False, val=9.80665)
    phase.add_design_parameter('Isp', units='s', opt=False, val=300.0)
    ...
    p['phase0.states:y'] = phase.interpolate(ys=[0, 100], nodes='state_input')
    p['phase0.states:vx'] = phase.interpolate(ys=[0, 0], nodes='state_input')
    p['phase0.states:vy'] = phase.interpolate(ys=[0, 500], nodes='state_input')

    p['phase0.design_parameters:g'] = 9.80665
    p['phase0.design_parameters:theta'] = 90.0
    p['phase0.design_parameters:mstruct'] = 100
    p['phase0.design_parameters:mprop'] = 50
...
if __name__ == '__main__':
    ballistic_rocket_max_range(transcription='gauss-lobatto', num_segments=10, run_driver=True,
