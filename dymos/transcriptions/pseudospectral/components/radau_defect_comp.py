    ...
    **kwargs : dict
    ...
    def configure_io(self, phase):
        ...
        for state_name, options in state_options.items():
            ...
            if not gd.compressed:
                # partials = np.array(num_segs-1
                ar = np.arange(num_col_nodes, dtype=int)
                r = np.repeat(np.arange(num_segs - 1, dtype=int), 2)
                c = gd.subset_node_indices['segment_ends'][1:-1]
                val = np.tile([-1., 1.], num_segs-1)
                self.declare_partials(of=var_names['cnty_defect'],
                                      wrt=var_names['val'], rows=r, cols=c, val=val)
    ...
    def compute(self, inputs, outputs):
        ...
        num_disc_nodes : int = gd.subset_num_nodes['state_disc']
        num_col_nodes : int = gd.subset_num_nodes['col']
        seg_end_idxs : int = gd.subset_node_indices['segment_ends']
