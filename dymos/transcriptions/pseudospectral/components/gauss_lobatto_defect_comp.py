...
class GaussLobattoDefectComp(om.ExplicitComponent):
    """
    Class definiton for the GaussLobattoDefectComp.

    GaussLobattoDefectComp computes the generalized defects for Gauss-Lobatto collocation.
    ...
    Unlike the Radau method, Gauss-Lobatto requires an interpolation step between the
    execution of the ODE at the discretization nodes and the ODE at the collocation nodes.
    During that interpolation, we compute the state values and rates needed to
    compute the defects.
    ...
    **kwargs : dict
    ...
    def configure_io(self, phase):
        ...
        for state_name in state_options:
            var_names[state_name] = {
                ...
                'staterate_col': f'staterate_col:{state_name}',
                'f_ode_col': f'f_ode_col:{state_name}',
        ...
        for state_name, options in state_options.items():
            ...
            self.add_input(var_names['staterate_col'],
                           shape=(num_col_nodes,) + shape,
                           units=rate_units,
                           desc='State rates as approximated by the gauss lobatto interpolation comp.')
            ...
            if rate_source_type != 'state':
                ...
                var_names['f_ode_col'] = self.var_names[rate_src_state]['val']
    ...
    def compute(self, inputs, outputs):
        ...
        for state_name, state_options in state_options.items():
            ...
            staterate_col = inputs[var_names['staterate_col']][rate_src_idxs] * rate_unit_conv
            f_ode_col = inputs[var_names['f_ode_col']][rate_src_idxs] * rate_unit_conv
            x = inputs[var_names['val']]
            x_0 = inputs[var_names['initial_val']]
            x_f = inputs[var_names['final_val']]

            outputs[var_names['rate_defect']] = staterate_col - (f_ode_col.T * dt_dstau).T
    ...
    def compute_partials(self, inputs, partials):
        ...
        for state_name, options in self.options['state_options'].items():
            ...
            partials[var_names['rate_defect'], var_names['f_ode_col']] = -np.repeat(dt_dstau, size) * rate_unit_conv
            partials[var_names['rate_defect'], 'dt_dstau'] = -f_ode_col.ravel()
