...
class GaussLobattoIterGroup(om.Group):
    ...
    **kwargs : dict
    ...
    def setup(self):
        ...
        # Discretization Node ODE

        ode_disc = _make_ode_system(ode_class=ode_class,
                                    num_nodes=num_disc_nodes,
                                    ode_init_kwargs=ode_init_kwargs,
                                    calc_exprs=self.options['calc_exprs'],
                                    parameter_options=self.options['parameter_options'])

        self.add_subsystem('ode_disc', subsys=ode_disc)

        # State interpolation

        self.add_subsystem('state_interp',
                           subsys=GaussLobattoInterpComp(grid_data=gd,
                                                         state_options=state_options,
                                                         time_units=time_options['units']),
                           promotes_inputs=['*'], promotes_outputs=['*'])

        # Collocation Node ODE

        ode_col = _make_ode_system(ode_class=ode_class,
                                   num_nodes=num_col_nodes,
                                   ode_init_kwargs=ode_init_kwargs,
                                   calc_exprs=self.options['calc_exprs'],
                                   parameter_options=self.options['parameter_options'])

        self.add_subsystem('ode_col', subsys=ode_col)
    ...
    def configure_io(self, phase):
        ...
        state_interp_comp.configure_io(phase)
        ...
        for name, options in state_options.items():
            ...
            if var_type == 'ode':
                self.connect(f'ode_col.{rate_source}', f'f_ode_col:{name}')


# class GaussLobattoIterGroupOld(om.Group):
#     """
#     Class definition for the GaussLobattoIterGroup.
...
#     When not utilizing solved segments, this just computes the defects.
