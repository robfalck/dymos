...
class GaussLobattoIterGroup(om.Group):
    ...
    **kwargs : dict
    ...
    def setup(self):
        ...
        self.add_subsystem('ode_disc', subsys=ode_class(num_nodes=ndn, **ode_init_kwargs))
        self.add_subsystem('state_interp', subsys=GaussLobattoInterpComp(grid_data=gd,
                                                                         state_options=state_options,
                                                                         time_units=time_options['units']),
                           promotes_inputs=['*'], promotes_outputs=['*'])
        self.add_subsystem('ode_col', subsys=ode_class(num_nodes=ncn, **ode_init_kwargs))
    ...
    def configure_io(self, phase):
        ...
        for name, options in state_options.items():
            ...
            for tgt in options['targets']:
                ...
                self.connect(f'state_col:{name}', f'ode_col.{tgt}')

            self.promotes('defects', inputs=((f'state_disc:{name}', f'states:{name}'),),
                          src_indices=om.slicer[state_src_idxs, ...])
            self.promotes('state_interp', inputs=((f'state_disc:{name}', f'states:{name}'),),
                          src_indices=om.slicer[state_src_idxs, ...])
            ...
            if var_type == 'ode':
                self.connect(f'ode_disc.{rate_source}', f'staterate_disc:{name}')
    ...
    def _get_rate_source_path(self, state_name, nodes, phase):
        ...
        if var_type == 't':
            ...
            k = 1 if phase.parameter_options[var]['static_target'] else gd.subset_num_nodes[nodes]
            node_idxs = np.zeros(k, dtype=int)
        else:
            # Failed to find variable, assume it is in the ODE
            rate_path = f'ode_disc.{var}'
            node_idxs = gd.subset_node_indices[nodes]
