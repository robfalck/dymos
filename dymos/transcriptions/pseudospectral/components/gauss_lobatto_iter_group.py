...
class GaussLobattoIterGroup(om.Group):
    ...
    Implements a Picard-style NLBGS algebraic loop for Gauss-Lobatto collocation:
      ...
      1. ``ode_all`` evaluates xdot at all nodes (disc + col) using current state estimates.
      2. ``lgl_interp_comp`` uses disc states + disc rates to update col state values via
         ...
         interpolated).
      3. ``states_all:{name}`` feeds back to ``ode_all`` targets (NLBGS algebraic loop).
      4. ``defects`` (CollocationComp) computes collocation residuals at col nodes.

    NLBGS converges in ~2 iterations:
      - Iter 1: ode_all evaluates disc + col states → lgl_interp_comp updates col estimates.
      - Iter 2: ode_all re-evaluates with corrected col states → same disc rates → converged.
    ...
    **kwargs : dict
    ...
    def setup(self):
        ...
        # --- 1. ODE at all nodes (must be added FIRST for NLBGS sweep order) ---
        ...
        # --- 2. LGL Hermite interpolation component (added SECOND for NLBGS sweep order) ---
        # Produces states_all:{name} (disc passthrough + col interpolated) and
        # staterate_col:{name} (interpolated rates at col nodes for defect f_approx).
        self.add_subsystem('lgl_interp_comp',
                           subsys=GaussLobattoInterpComp(
                               grid_data=gd,
                               state_options=state_options,
                               time_units=time_options['units']))

        # --- 3. Collocation + boundary-state defects ---
        # GaussLobattoDefectComp handles both:
        #   (a) collocation defects: defects:{name} = (f_approx - f_computed) * dt_dstau
        #   (b) boundary defects: initial/final_state_defects:{name} = initial/final_states - states_all[0/-1]
        # Constraints (== 0) are added for non-solve_segments states.
        ...
        # --- 4. InputResidsComp for solve_segments (optional) ---
    ...
    def configure_io(self, phase):
        ...
        # dt_dstau at all nodes; slice to col nodes for defects and lgl_interp_comp
        self.promotes('defects', inputs=('dt_dstau',),
                      src_indices=om.slicer[col_idxs, ...],
                      src_shape=(nn,))
        self.promotes('lgl_interp_comp', inputs=('dt_dstau',),
                      src_indices=om.slicer[col_idxs, ...],
                      src_shape=(nn,))
        ...
        for name, options in state_options.items():
            ...
            # Promote lgl_interp_comp.state_disc:{name} → states:{name} at group level,
            # expanding from n_input nodes to n_disc nodes via state_input_to_disc map.
            # This makes states:{name} visible as a group-level input (n_input nodes).
            self.promotes('lgl_interp_comp',
            ...
            # Promote states_all:{name} to group level for timeseries access and ode connection.
            self.promotes('lgl_interp_comp', outputs=[f'states_all:{name}'])

            # states_all:{name} → ode_all.{target} (NLBGS algebraic loop)
            # This must happen for ALL states regardless of rate source type.
            for tgt in options['targets']:
                self.connect(f'states_all:{name}',
                             f'ode_all.{tgt}')

            # lgl_interp_comp.staterate_col:{name} → defects f_approx:{name}
            # Hermite-interpolated rates are always used as f_approx in GL collocation.
            self.connect(f'lgl_interp_comp.staterate_col:{name}',
                         f'f_approx:{name}')

            if var_type == 'ode':
                # ode_all.{rate_source}[disc_idxs] → lgl_interp_comp.staterate_disc:{name}
                self.connect(f'ode_all.{rate_source}',
                             f'lgl_interp_comp.staterate_disc:{name}',
                             src_indices=om.slicer[disc_idxs, ...])

                # ode_all.{rate_source}[col_idxs] → defects f_computed:{name}
                self.connect(f'ode_all.{rate_source}', f'f_computed:{name}',
                             src_indices=om.slicer[col_idxs, ...])
            # else: non-ODE rate sources (parameter, time, control, state) are connected
            # by GaussLobattoNew.configure_defects via phase-level connections.
