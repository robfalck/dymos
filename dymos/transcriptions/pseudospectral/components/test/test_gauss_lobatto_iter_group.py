...
from dymos.utils.misc import GroupWrapperConfig, CompWrapperConfig
from dymos.transcriptions.pseudospectral.components.gauss_lobatto_iter_group import GaussLobattoIterGroup
...
GaussLobattoIterGroupWrapped = GroupWrapperConfig(GaussLobattoIterGroup, [PhaseStub()])
TimeCompWrapped = CompWrapperConfig(TimeComp)
            ...
            for direction in ['forward',]:# 'backward']:
                for compressed in [True,]:# False]:
                    with self.subTest(msg=f'{direction=} {compressed=}'):
                        ...
                        grid_data = GaussLobattoGrid(num_segments=2, nodes_per_seg=8, compressed=compressed)
                        ...
                        p.model.add_subsystem('time_comp',
                                              TimeCompWrapped(num_nodes=nn, node_ptau=grid_data.node_ptau,
                                                              node_dptau_dstau=grid_data.node_dptau_dstau, units='s'))

                        p.model.add_subsystem('lgl', GaussLobattoIterGroupWrapped(state_options=state_options,
                                                                                  time_options=time_options,
                                                                                  grid_data=grid_data,
                                                                                  ode_class=ode_class))

                        # p.model.connect('time_comp.dt_dstau', 'lgl.dt_dstau')

                        lgl = p.model._get_subsystem('lgl')

                        lgl.nonlinear_solver = om.NewtonSolver(solve_subsystems=True, iprint=-1)
                        lgl.linear_solver = om.DirectSolver()

                        p.setup(force_alloc_complex=True)

                        # Instead of using the TimeComp just transform the node segment taus onto [0, 2]
                        times = grid_data.node_ptau + 1
                        times_disc = times[grid_data.subset_node_indices['state_disc']]
                        times_col = times[grid_data.subset_node_indices['col']]
                        ...
                        # p.set_val('lgl.dt_dstau', (times[-1] / 2.0 / grid_data.num_segments))

                        # if direction == 'forward':
                        #     p.set_val('lgl.initial_states:x', 0.5)
                        # else:
                        #     p.set_val('lgl.final_states:x', solution[-1])

                        # p.set_val('lgl.states:x', 0.0)
                        # p.set_val('lgl.ode_disc.t', times_disc)
                        # p.set_val('lgl.ode_col.p', times_col)

                        # p.run_model()

                        # x = p.get_val('lgl.states:x')
                        # x_0 = p.get_val('lgl.initial_states:x')
                        # x_f = p.get_val('lgl.final_states:x')
        ...
        with dymos.options.temporary(include_check_partials=True):
            for direction in ['forward']:#, 'backward']:
                with self.subTest(msg=f'{direction=}'):
                    ...
                    time_options = TimeOptionsDictionary()
                    grid_data = GaussLobattoGrid(num_segments=5, nodes_per_seg=3, compressed=False)
                    nn = grid_data.subset_num_nodes['all']
                    ...
                    lgl.nonlinear_solver = om.NewtonSolver(solve_subsystems=True, maxiter=100,
                                                           iprint=-1, atol=1.0E-10, rtol=1.0E-10)
                    lgl.nonlinear_solver.linesearch = om.ArmijoGoldsteinLS(bound_enforcement='vector')
                    lgl.linear_solver = om.DirectSolver()
                    ...
                    # Each segment is of the same length, so dt_dstau is constant.
                    # dt_dstau is (tf - t0) / 2.0 / num_seg
                    # dt_dstau = np.ones_like(times) * times[-1] / 2.0 / grid_data.num_segments
                    # print('setting lgl.dt_dstau', dt_dstau)
                    p.set_val('lgl.dt_dstau', 0.2)

                    if direction == 'forward':
                        p.set_val('lgl.initial_states:z', [0.5, 0.0])
                    else:
                        p.set_val('lgl.final_states:z', solution[-1], indices=om.slicer[:, 0])
                        p.set_val('lgl.final_states:z', 20.0, indices=om.slicer[:, 1])

                    p.set_val('lgl.states:z', 0.0)
                    p.set_val('lgl.ode_disc.t', times[grid_data.subset_node_indices['state_disc']])
                    p.set_val('lgl.ode_col.t', times[grid_data.subset_node_indices['col']])
                    p.set_val('lgl.ode_disc.p', 1.0)
                    p.set_val('lgl.ode_col.p', 1.0)

                    p.run_model()

                    x = p.get_val('lgl.states:z')
                    x_0 = p.get_val('lgl.initial_states:z')
                    x_f = p.get_val('lgl.final_states:z')
            ...
            for direction in ['forward', 'backward']:
                for compressed in [True, False]:
                    with self.subTest(msg=f'{direction=} {compressed=}'):
                        ...
                        if compressed:
                            # Solution is defined at all nodes, need to only provide the input node values.
