...
def _newton_coefs(x, y):
    """
    Given an array of monotonically increasing values x, form
    the lower triangular Newton basis matrix for interpolation.

    Parameters
    ----------
    x : np.array
        Location of the interpolation data in the independent axis.

    Returns
    -------
    N : np.array
        A lower triangular matrix of the newton interpolation basis.
    """
    n = x.size
    coef = np.zeros((n, n))
    # coef2 = np.zeros((n, n))
...
def newton_interp(a, x_data, x):
    ...
    el = x - x_data
    # print('el')
    # g = [1, el[0], el[0] * el[1], el[0] * el[1] * el[2]]
    # g = np.ones(n + 1)
    # print(el)
    # g[1:] = np.cumprod(el[1:])
    # print(g)

    # p = a[0] + el[0] * a[1] + el[0] * el[1] * a[2] + el[0] * el[1] * a[3]
    g = np.cumprod(el[:-1])
    p = a[0] + np.dot(g, a[1:])

    dp_da = [1, g[0], g[1], g[2]]
    dp_dg = a[1:]
    dg_del = np.array([[            1,             0,             0],
                       [el[1]        , el[0]        ,             0],
...
class NewtonControlInterpComp(om.ExplicitComponent):
    """
    A component which interpolates control values in 1D using Newton interpolation.
    ...
    **kwargs
    ...
    def _configure_controls(self):
        ...
        for seg_idx in range(gd.num_segments):
            ...
            if seg_control_order not in self._N_hat:
                self._N_hat[seg_control_order] = np.vander(control_disc_seg_stau, increasing=True)
                self._N_hat_inv[seg_control_order] = np.linalg.inv(self._N_hat[seg_control_order])
            # if seg_control_order + 1 not in self._fac:
            #     self._fac[seg_control_order + 1] = np.arange(seg_control_order + 1, dtype=int)
